...
const exportUsers =async(req,res)=>{
  const userPhone = req.body.userList?req.body.userList:''
  ...
  const enDate = p2e(dateNow).replace(/\D/g,'-')
    ...
    var userRaw = []
	console.log(userPhone&&userPhone.length)
	
    var userList= []
	if(userPhone)
		userList=await user.find({phone:{$in:userPhone}}).lean()
	else
		userList=await user.find({}).lean()
