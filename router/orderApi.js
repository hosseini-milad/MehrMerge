...
router.post('/stock/adminlist', jsonParser, async (req, res) => {
    ...
    try {
        ...
        res.json({
            stock: pagingData, brandList: brandList, lenzIndexList: lenzIndexList,
...
router.post('/manufacture/list', jsonParser, async (req, res) => {
    ...
    try {
        ...
        res.json({
            manData: pageData, manufacture: manufactureList,
...
router.post('/manufacture/add', jsonParser, async (req, res) => {
    ...
    try {
        ...
        if (existMan.length) {
            ...
            saveHesabFa = await HesabFaApiCall("/item/save", {
                item: {
                    Name: lensTitle,
            ...
            if (req.body.sku) {
                ...
                hesabFa = await HesabFaApiCall("/item/save", {
                    item: {
                        Name: lensTitle,
                        ItemType: 0,
                ...
                const hesabFa = await HesabFaApiCall("/item/save", {
                    item: {
                        Name: lensTitle,
                        ItemType: 0,
                        ProductCode: lastSku,
    ...
    try {
        ...
        if (existOrderItem) {
            ...
            if (data.status === "inprogress") {
                ...
                if (!errors) {
                    rxData = await RXSchema.updateOne({ userId: data.userId, status: "initial" },
...
router.post('/addstock', jsonParser, auth, async (req, res) => {
    ...
    try {
        ...
        const userData = await userSchema.findOne({ _id: data.userId })
        var credit = userData.profile === "659b9ce3d9c3154d2f94a82e" ?
            await calcCreditUser(userData) : { credit: 1 }
        //console.log(credit)
        if (!credit || credit.credit < 1) {
            res.json({
                error: `مشتری گرامی<br/>
        اعتبار: ${credit.remainCredit}<br/>
        FOB: ${credit.remainFob}<br/>
        کسری اعتبار: ${credit.credit * -1}<br/>
        لطفا نسبت ایجاد تراکنش به صورت یارانه ای و یا غیر یارانه ای به میزان 
        ${credit.credit * -1} اقدام نمایید`
                , errorTitle: "اعتبار کافی نیست"
            })
            return
        }
        ...
        try { const taskData = await CreateTask("order", data, userData.cName) } catch { }
...
router.get('/stockSeprate', jsonParser, async (req, res) => {
    ...
    try {
        ...
        //console.log(userData)
        res.json({
            stockDataInprogress: stockDataInprogress,
    ...
    try {
        ...
        if (data.status === "completed")
            ...
            if (data.status === "delivered") {
                ...
                setFaktor = await HesabFaApiCall("/invoice/save", {
                    invoice: {
...
router.post('/manage/addstock', jsonParser, auth, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (data.status.includes("cancel")) {
                const newUserLog = await logSchema.create({
                    ...
                    user: userData._id,
                    kind: "customer",
                    description: "سفارش با شماره " + req.body.stockOrderNo +
                        " لغو شده است ",
                    status: "unread",
                    date: Date.now()
                })
                const smsResult = await sendSmsUser(userData._id, process.env.cancelOrder,
                    req.body.stockOrderNo, "token2")
                //console.log(smsResult)
            }
        } catch { }
        ...
        await tasks.updateOne({ orderNo: req.body.stockOrderNo },
            { $set: { taskStep: data.status } })
        await orderLogSchema.create({
            status: data.status, rxOrderNo: req.body.stockOrderNo,
...
router.get('/manage/rxList', jsonParser, auth, async (req, res) => {
    ...
    try {
        ...
        const userData = await userSchema.findOne({ _id: req.headers['userid'] })
        res.json({
            inprogress: inprogressList, accept: acceptList,
...
const cartCreator = async (cartItemsRaw, userId) => {
    ...
    return ({
        cart: regularCart.concat(freeCart),
...
router.get('/inqueryCredit', auth, async (req, res) => {
    try {
        ...
        const result = { credit: balance }
        const updateResponse = await CreditSchema.updateOne({ phone }, { $set: result }, { upsert: true })
...
function CallApi(token) {
    ...
    const { SERP_API_Address } = process.env
    ...
    this.post = async (url, body) => {
        return await axios.post(`${SERP_API_Address}${url}`, body, config)
            .then(response => { return response.data })
            .catch(error => { throw new Error(error.message) })
    }
    this.get = async (url) => {
        return await axios.get(`${SERP_API_Address} ${url}`, config)
            .then(response => { return response.data })
            .catch(error => { throw new Error(error.message) })
    }
}
async function getToken() {
    ...
    const { SERP_API_USERNAME, SERP_API_PASSWORD, SERP_API_Address } = process.env

    var qs = require('qs');
    var data = qs.stringify({
        password: SERP_API_PASSWORD,
        username: SERP_API_USERNAME
    });
    var config = {
        method: 'post',
        url: `${SERP_API_Address}users/Token`,
        headers: {
